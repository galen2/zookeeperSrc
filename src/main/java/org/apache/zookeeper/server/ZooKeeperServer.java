...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    /**
     * 每个新连接的客户端，都会依据服务器规则，创建相应的sessionId。
     * 然后在processPacket函数最终拼接request对象时候使用
     */
    ...
    public void submitRequest(Request si) {
        ...
        try {
            ...
            if (validpacket) {
            	//会依据leader或者follow创建不同的processor，这里就确定了读写操作转移到leader还是follow得逻辑
    ...
    /**
     * 这里是真正的解析客户端请求数据包
     * incomingBuffer里面包含key为header和request的数据类型，
     * 从流里面读出header数据之后，剩下的自然就是request数据。
     * 数据的在客户端的拼接：ClientCnxn--》Packet 的createBB函数
     */

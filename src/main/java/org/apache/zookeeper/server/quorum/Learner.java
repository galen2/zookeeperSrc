...
public class Learner {       
     ...
     * @throws IOException
     * 重点：follow节点若接收到对数据修改操作，均又这里通过socket流的形势，转发给leader节点，让leader处理
     */
    void request(Request request) throws IOException {
        ...
        oa.close();
        /**
         * 组建REQUEST类型的消息包，表示follow发送过去的请求。最终请求会在leader的LearnerHandler的613行处理这些请求
         */
     ...
     * 如：192.168.33.14:2888:3888  数据交互端口号：2888。3888为选举时候的端口号
     * @throws IOException - if the socket connection fails on the 5th attempt
     * @throws ConnectException
     * @throws InterruptedException
     * 重点：此函数会创建从follow连接到leader端口号2888节点socket的连接，最终构建出leaderOs的socket输出流，
     * 随后和leader节点的数据交互就通过这个输出流来传输数据给leader。
     */

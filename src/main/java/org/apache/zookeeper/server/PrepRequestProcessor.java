 ...
 * 1、此类很重要的一点是根据不同的操作类型设置 request的txn变量值，即客户端请求的数据内容
 * 2、更新操作会设置辞职，其余如查询操作此值为空，下一个ProposalRequestProcessor拿到请求之后，
 * 如果发现不为空即为更新操作，则需要往其他子节点同步数据。反之，若不是更新操作，则无需同步数据，直接往后面责任链处理即可
 */
public class PrepRequestProcessor extends Thread implements RequestProcessor {
    ...
    public void run() {
        try {
            while (true) {
            	//所有对数据修改操作，都会有这个请求出来完成。这个处理类也是属于leader节点的
     ...
     * @param request
     * @param record
     * 组装对节点发生修改的数据，最终调用addChangeRecord方法，存放集合里面，最终被FinalRequestProcessor下个处理逻辑
     */

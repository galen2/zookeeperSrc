...
public class FileTxnLog implements TxnLog {
        ...
        throws IOException
    {
        if (hdr != null) {
            ...
            if (logStream==null) {
               //第一次事务请求，创建新文件，然后在新文件写入FileHeader文件头
            ...
            /**
             * 第二次事务请求，写入数据。oa底层用的还是logStream数据流，
             * 由于并没有执行logStream.flush()函数，因此，此时数据还在BufferedOutputStream的缓存里面，
             * 并没有持久化到磁盘
             */
     ...
     * @throws IOException
     * 这个函数很关键，在事务日志文件写入之后，立马增加文件的大小，实际就是创建一个大的文件里面内容都是空的，
     * 这是为了给文件分配连续的磁盘空间，减少后续写入和读取文件磁盘寻到的开销，提高文件系统的写数据效率。
     */
